# function to check no of parity bits in generation of hamming code
...
def hammingCodes(path):
    filename, file_extension = os.path.splitext(path)
    output_file = "2encrypted.bin"
...
def hammingCorrection(path):
    filename, file_extension = os.path.splitext(path)
    output_file = "4decrypted.bin"
    ...
    while i < n:
        ...
        if total % 2 > 0:
            ...
            errorBit = errorBit + k  # to check even parity summing up all the elements in sublist and if summ is even than even parity else odd parity
        i += 1
    if errorBit >= 1:
        print("error in ", errorBit, " bit after correction data is ")
